...
function PLAYER:SetCurrency(amount)
    ...
    mp.mysql.WriteVar(self:SteamID64(),"mpdonate_players","balance",tonumber(amount) or 0)
end
...
function PLAYER:AddItem(class)
    ...
    if not slotid then
        ...
        mp.mysql.AddPlayerInventory(self:SteamID64(),class,exp)
    else
        self.mp_donate[slotid]["expires"] = os.time() + mp.GetItem(class):GetExpires() -- Кароче в чем прикол, нахуй нам еще создавать предметы, если можно просто перезаписать Expires?
    end
    mp.PlayerInventorySync(self)
end
...
function PLAYER:TakeItem(class)
    ...
    if slotid then
        ...
        mp.mysql.TakePlayerInventory(self:SteamID64(),class)
    end
    mp.PlayerInventorySync(self)
end
...
function PLAYER:CheckExpires()
    ...
    if self.mp_donate then
        for k,v in pairs(self.mp_donate) do -- парсим все итемы которые знаем
            if (v['expires'] < os.time() and v['expires'] ~= 0) then -- если значет дата всё, тю-тю
                ...
                mp.mysql.TakePlayerInventory(self:SteamID64(),v['class'])
            end
            if haveitems[v['class']] then
                mp.mysql.TakePlayerInventoryByID(self:SteamID64(),v['id'])
                edited = true
            end
            haveitems[v['class']] = true
        end
    end
    if edited then mp.PlayerInventorySync(self) end
    ...
    if d_expires > os.time() then
        ...
        --     mp.Msg(self:SteamID64().." usergroup recovery to "..d_usergroup)
        -- end
    else
        -- if d_usergroup == usergroup then
            self:AdvancedSetUserGroup("user")
            mp.mysql.WriteVarServers(self:SteamID64(),"mpdonate_groups","expires",0)
            mp.Msg(self:SteamID64().." usergroup expires end setting to DEFAULT(user)")
            mp.mysql.Log(self:GetDonateToken(),"Вы были с привилегии "..d_usergroup.." так как срок закончен",self:SteamID64())
        -- end
    end
end
...
function PLAYER:InitDonate()
    ...
    self.mp_donate = {}
    mp.mysql.GetPlayer(sid,function(data)
        if data then
            mp.mysql.GetPlayerInventory(sid,function(inv)
                if inv then
                    self.mp_donate = inv
                else
                    mp.DebugMsg(sid.." inventory not found, using default")
                end
                timer.Simple(1,function() mp.PlayerInventorySync(self) end)
            end)
            mp.DebugMsg(sid.." SETTING VAR DonateBalance From Data Base VALUE: "..(data["balance"] or "(БД ВЕРНУЛА NULL ЗНАЧЕНИЕ)") )
            self:SetNetVar("DonateBalance", tonumber(data["balance"] or 0) or 0)
            mp.mysql.GetPlayerGroup(sid,function(data2)
                if data2 then
                    mp.DebugMsg(sid.." SETTING VAR DonateUserGroupExpires From Data Base VALUE: "..(data2["expires"] or "(БД ВЕРНУЛА NULL ЗНАЧЕНИЕ)"))
                    self:SetNetVar("DonateUserGroupExpires", tonumber(data2["expires"] or 0) or 0)
                    self.donategroup = data2["usergroup"] or "user"
                else
                    mp.mysql.MakePlayer(sid,0,token,true)
                end
            end)
            self.donatetoken = data["pltoken"]
        else
            local token = tokengeneration(sid)
            mp.mysql.MakePlayer(sid,0,token)
            mp.DebugMsg(sid.." SETTING VAR DonateBalance VALUE: 0")
            self:SetNetVar("DonateBalance", 0)
            mp.DebugMsg(sid.." SETTING VAR DonateUserGroupExpires VALUE: 0")
            self:SetNetVar("DonateUserGroupExpires", 0)

            self.donategroup = "user"
            self.donatetoken = token
            mp.DebugMsg(sid.." user not found, using default")
        end
